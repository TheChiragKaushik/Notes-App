...
export const githubData = async () => {
  const userResponse = await fetch('https://api.github.com/users/thechiragkaushik');
  const reposResponse = await fetch('https://api.github.com/users/TheChiragKaushik/repos');
  ...
  return (
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 p-4">
        {reposData.map(repo => (
          <>
          <div key={repo.id} className="bg-gray-800 p-4 rounded-md text-white shadow-md">
            <div>
            <h1 className="text-xl font-bold">{repo.name}</h1>
            <p>{repo.description}</p>
            </div>
            <div className="p-1 mt-1">
            <a className="bg-black p-2 rounded-md" href={repo.html_url} target="_blank">View</a>
            </div>
          </div>
          </>
        ))}
